...
class DataSeg {
 ...
 public:
  ...
  [[nodiscard]] inline uint16_t GetShort(uint32_t nIndex) const {
    ...
    return *(reinterpret_cast<uint16_t *>(data + nIndex - startOff));
  }

  [[nodiscard]] inline uint32_t GetWord(uint32_t nIndex) const {
    assert((nIndex >= startOff) && (nIndex + 4 <= endOff));
    return *(reinterpret_cast<uint32_t *>(data + nIndex - startOff));
  }

  [[nodiscard]] inline uint16_t GetShortBE(uint32_t nIndex) const {
    assert((nIndex >= startOff) && (nIndex + 2 <= endOff));
    return (static_cast<uint8_t>(data[nIndex - startOff]) << 8) + static_cast<uint8_t>(data[nIndex + 1 - startOff]);
  }

  [[nodiscard]] inline uint32_t GetWordBE(uint32_t nIndex) const {
    assert((nIndex >= startOff) && (nIndex + 4 <= endOff));
    return static_cast<uint8_t>(data[nIndex - startOff] << 24) + static_cast<uint8_t>(data[nIndex + 1 - startOff] << 16)
        + static_cast<uint8_t>(data[nIndex + 2 - startOff] << 8) + static_cast<uint8_t>(data[nIndex + 3 - startOff]);
  }
